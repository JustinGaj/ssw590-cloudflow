...
          cleanWs()
          // Clone the project into the host workspace (needed for artifacts later)
          checkout scm
        }
      }
    }
...
      steps {
            // FIX: Removed the outer 'set -eux' to prevent Groovy from misinterpreting shell history.
            // The inner shell command retains 'set -eux' for clean execution inside the container.
            sh '''
...
        }
        // Uses docker:latest to bypass GLIBC issues and builds directly from the Git URL.
        sh '''
...
          // Mount /tmp to share the artifact, then clone and build inside the LaTeX container.
...
          "
          // Move the created artifact from /tmp back into the workspace.
...
          // zip fails if latex.pdf doesn't exist, so we need to make it optional.
...
        sh '''
          set -eux
          // Uses docker:latest wrapper to send commands to the host Docker daemon via socket.
          docker run --rm -u 0 -v /var/run/docker.sock:/var/run/docker.sock docker:latest sh -c "
